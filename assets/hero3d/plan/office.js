...
export const OFFICE_BLOCKS = [
  ...
  { name: 'C.b3', cols: { x: [36.0, 54.2], z: [23.0, 31.4] }, edges: { w: 3.0, e: 0.6, n: 0.6, s: 3.0 } },  // 2.4 m past b2 west + south
].map((b, k) => ({
  ...b,
...
export function officeMasses() {
  ...
  const blocks = OFFICE_BLOCKS.map((b, k) => {
    ...
    return box(b.name, x0, x1, z0, z1, 0, 3 * OFFICE_FLOOR, 'concrete', 'C', [0.372 + k * 0.040, 0.042], k ? OFFICE_BLOCKS[k - 1].name : 'C.baseE', FLOORS);
  });
  return [
    ...
    ...blocks,
    box('C.screen', 44, 54, 24, 30.6, 0, 3.2, 'frame', 'C', [0.494, 0.006], 'C.b3', { glaze: GLAZE.screen }),
  ];
}
...
export const OFFICE_TERRACES = [
  ...
  { name: 'L.o2E', rect: [55.4, 69.8, 14.4, 31.6], y: OFFICE_BLOCK_TOPS[1], host: 1 },
  { name: 'L.o2N', rect: [35.8, 55.4, 14.4, 21.8], y: OFFICE_BLOCK_TOPS[1], host: 1 },
  { name: 'L.o3', rect: [33.4, 54.4, 22.8, 34.0], y: OFFICE_BLOCK_TOPS[2], host: 2 },
];
...
export const OFFICE_PLANTERS = [];
{
  ...
  for (const [z0, z1] of seg(15.0, 42.0)) OFFICE_PLANTERS.push({ rect: [72.6, 73.8, z0, z1], y: B, hang: { face: 'e', at: 74.05, over: 'screen', max: 4.5 } });
...
export function officeParts(tier, rand) {
  ...
  for (const b of OFFICE_BLOCKS) {
    const [x0, x1, z0, z1] = b.rect;
    const o = 0.5;
    block(x0 - o, x1 + o, b.y1 - 0.8, b.y1, z0 - o, z1 + o, 'frame');
    block(x0 - o, x1 + o, b.y0, b.y0 + 0.6, z0 - o, z1 + o, 'frame');
    for (let k = 1; k < 3; k++) {
      const y = b.y0 + k * OFFICE_FLOOR;
      block(x0 - 0.3, x1 + 0.3, y - 0.35, y, z0 - 0.3, z1 + 0.3, 'frame');
    }
    for (const [cx, cz] of [[x0, z0], [x1, z0], [x0, z1], [x1, z1]]) block(cx - o, cx + o, b.y0, b.y1, cz - o, cz + o, 'frame');
  }

  // --- warm timber soffits beneath the projecting edges ----------------------------------
  block(68.3, 70.7, 26.94, 27.0, 13.6, 32.4, 'woodLight');                 // b2, east
  block(32.5, 34.9, 39.54, 39.6, 21.9, 32.5, 'woodLight');                 // b3, west
  block(32.5, 55.3, 39.54, 39.6, 32.5, 34.9, 'woodLight');                 // b3, south
  ...
  louvre('w', 23.4, 33.6, 33.0, t2 + 0.6, t3 - 0.8);
  ...
  // b2 east terrace: tables under umbrellas and a bench
  K.umbrella(60.0, 22.0, t2 + 0.3, 2.8); K.dining(60.0, 22.0, t2 + 0.3, 4);
  K.umbrella(64.2, 27.0, t2 + 0.3, 2.8); K.dining(64.2, 27.0, t2 + 0.3, 4);
